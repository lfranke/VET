...
layout(location = 12) uniform int render_cam_model = 0;
...
out float discard_float;

#define M_PI 3.14159265358979323846
vec4 toPano(vec3 p)
{
    float phi = atan(normalize(p).z, normalize(p).x) / (M_PI);
    float chi = -(acos(normalize(p).y) - M_PI / 2) / 2;
    return vec4(phi, chi, length(p)/100, 1);
}
...
void main() {
  ...
  // // world_p.z*=-1;
    ...
    if (dot(normalize(view_p), view_n) > 0 && render_cam_model == 0)
    {
    ...
    vec2 norm_p = gl_Position.xy/gl_Position.w;
    if(norm_p.x < -1 || norm_p.y < -1 || norm_p.x > 1 || norm_p.y > 1)
        discard_float = 100;

    if (render_cam_model == 2)
        gl_Position = toPano(vec3(cv_view * model * world_p));
  //  if (render_cam_model == 2)
  //      gl_Position = toPano(vec3(cv_view * model * world_p));
...
in vec4 v_data;
flat in int v_index;
in float discard_float;
layout(location=9) uniform float color_scale = 1;
layout(location=10) uniform int display_mode = 0;
layout(location=11) uniform float cutoff_val = 0;
