...
void RealTimeRenderer::imgui()
{
    ...
    if (ImGui::Begin("Neural Renderer"))
    {
        ...
        /*
        if (ImGui::ListBoxHeader("###RenderModes", 4))
        {
            std::vector<std::string> render_modes_str = {"DT", "Blend", "FuzzyBlend", "BilinearBlend"};
...
void RealTimeRenderer::Render(ImageInfo fd, vec3 debug_refl_dir)
{
    ...
    pipeline->render_module->params->pipeline_params.enable_environment_map = render_env_map;
