...
void NeuralPointCloudCudaImpl::UpdateCellStructureForPointOptim(float size_of_box_in_ws, AABB custom_aabb)
{
    ...
    for (int z_n = 0; z_n < num_slices.z(); ++z_n)
    {
        for (int y_n = 0; y_n < num_slices.y(); ++y_n)
        {
            ...
            {
                uint index        = z_n * num_slices.y() * num_slices.x() + y_n * num_slices.x() + x_n;
...
#pragma omp parallel for
    ...
    t_cell_bb_min =
        torch::from_blob(bb_mins, {(long)full_point_batches + 1, 3}, torch::TensorOptions().dtype(torch::kFloat32))
            .clone()
            .cuda()
            .contiguous();
    t_cell_bb_length =
        torch::from_blob(bb_lengths, {(long)full_point_batches + 1, 3}, torch::TensorOptions().dtype(torch::kFloat32))
            .clone()
            .cuda()
            .contiguous();
}
