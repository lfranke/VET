    ...
    : scene(scene), params(_params)
{
    ...
    if (params->pipeline_params.enable_environment_map && !params->pipeline_params.use_points_for_env_map)
    {
        environment_map =
            EnvironmentMap(params->pipeline_params.env_map_channels, params->pipeline_params.env_map_w,
                           params->pipeline_params.env_map_h, params->pipeline_params.log_texture, 0,
                           params->pipeline_params.env_spheres, params->pipeline_params.env_inner_radius,
                           params->pipeline_params.env_radius_factor, params->pipeline_params.non_subzero_texture);
    }
    ...
    if (((params->pipeline_params.use_points_for_env_map && params->pipeline_params.use_env_points_at_start) ||
        std::filesystem::exists(params->train_params.checkpoint_directory + "/texture.pth")) && !eval_only)
    {
        {
            AddNewRandomForEnvSphere(params->pipeline_params.env_spheres, params->pipeline_params.env_inner_radius,
                                     params->pipeline_params.env_radius_factor, params->pipeline_params.env_num_points,
                                     false);
        }
    }
...
void NeuralScene::CreateStructureOptimizer()
{
    ...
    {
        std::cout << "optimizing pinhole intrinsics with lr " << params->optimizer_params.lr_intrinsics << std::endl;
...
void NeuralScene::ShrinkTextureOptimizer(torch::Tensor indices_to_keep)
{
    if (params->optimizer_params.texture_optimizer == "adam")
    {
        ...
        adam->shrinkenInternalState(0, indices_to_keep);
        adam->shrinkenInternalState(2, indices_to_keep);
    }
}
    ...
    else if (params->optimizer_params.texture_optimizer == "sgd")
    {
        ...
        if (!params->optimizer_params.fix_texture)
        {
            ...
            }
            {
                auto opt_t = std::make_unique<TexOpt>(params->optimizer_params.lr_texture);
                std::vector<torch::Tensor> ts;
                ts.push_back(texture->confidence_value_of_point);
                g.emplace_back(ts, std::move(opt_t));
            }
        }
...
void NeuralScene::Log(const std::string& log_dir)
{
    ...
    PrintTensorInfo(texture->confidence_value_of_point);
    std::cout << "  Confidences under " << params->points_adding_params.removal_confidence_cutoff << ": "
              << torch::count_nonzero(torch::where(texture->confidence_value_of_point <
                                                       params->points_adding_params.removal_confidence_cutoff,
                                                   torch::ones_like(texture->confidence_value_of_point),
                                                   torch::zeros_like(texture->confidence_value_of_point)))
                     .item<int>()
              << std::endl;
                                                ...
                                                float ct_volume_scale, vec3 ct_volume_translation)
{
    ...
    std::vector<TemplatedImage<u_int8_t>> png_stack;
    auto getPx = [](TemplatedImage<u_int8_t>& image, int x, int y) { return float(uint(image(x, y))) / float(255); };
    for (auto& filename : png_paths)
    {
        std::cout << filename << std::endl;
        png_stack.emplace_back(filename);
    }
