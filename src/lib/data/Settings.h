...
struct PipelineParams : public ParamsBase
{
    ...
    void Params(ParamIterator* it)
    {
        ...
        SAIGA_PARAM(use_points_for_env_map);
        SAIGA_PARAM(env_num_points);
        SAIGA_PARAM(use_env_points_at_start);

        SAIGA_PARAM(env_map_w);
        SAIGA_PARAM(env_map_h);
        SAIGA_PARAM(env_map_channels);
        SAIGA_PARAM(env_inner_radius);
        SAIGA_PARAM(env_radius_factor);
        SAIGA_PARAM(env_spheres);
        SAIGA_PARAM(env_axis);
    ...
    bool use_env_points_at_start = false;
    ...
    std::vector<int> render_modes_start_epochs = {-1, 0, -1, -1};
};
...
struct PointAddingParams : public ParamsBase
{
    ...
    void Params(ParamIterator* it)
    {
        ...
        // SAIGA_PARAM_LIST(custom_aabb_min_str, ' ');
        // SAIGA_PARAM_LIST(custom_aabb_max_str, ' ');
        /*
                if (custom_aabb_min_str.size() < 3)
                {
                    for (int i = 0; i < 3; ++i)
                    {
                        custom_aabb_min[i] = 0.0;
                        custom_aabb_max[i] = 0.0;
                    }
                }
                else
                {
                    for (int i = 0; i < 3; ++i)
                    {
                        custom_aabb_min[i] = std::stof(custom_aabb_min_str[i]);
                        custom_aabb_max[i] = std::stof(custom_aabb_max_str[i]);
                    }

                    std::cout << "Custom AABB is set, NeAT scene scale is ignored!" << std::endl;
                }
                */
    ...
    // int switch_to_fuzzy_rendering_after_epoch = 900;
    ...
    // std::vector<std::string> custom_aabb_min_str = {"0.0", "0.0", "0.0"};
    // vec3 custom_aabb_min                         = vec3(0, 0, 0);
    // std::vector<std::string> custom_aabb_max_str = {"0.0", "0.0", "0.0"};
    // vec3 custom_aabb_max                         = vec3(0, 0, 0);
