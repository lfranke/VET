    ...
    : log_texture(log_texture), random_init(random_init)
{
    if (random_init)
    {
        ...
        float factor = 2;
        texture_raw  = (torch::rand({num_channels, num_points}) * factor);
        texture_raw  = texture_raw - torch::ones_like(texture_raw) * (factor / 2);

        texture_raw = torch::empty({num_channels, num_points});
        texture_raw.uniform_(0, 1);
    }
    else
    {
        texture_raw = torch::ones({num_channels, num_points}) * 0.5f;
    }
    texture = texture_raw.clone();

    background_color_raw = torch::ones({num_channels}) * -1;

    confidence_value_of_point = torch::ones({1, num_points}) * 0.5;
    confidence_raw            = torch::ones({1, num_points}) * 0.5;

    if (log_texture)
    {
        std::cout << "initialized first texture channel (before checkpoint loading) with 1" << std::endl;
        texture_raw      = torch::log(torch::ones({num_channels, num_points}) * 0.5);
